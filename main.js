...
var srtb = "photolink";
var srtd = "desc";
var nowr = ["maxdate", "discoverdate"];
var nwnm = ["download", "spectralink", "photolink", "radiolink", "xraylink"];
var revo = ["photolink", "spectralink", "radiolink", "xraylink"];
var plen = [10, 50, 250];
var shrt = "SNe";
var sing = "Supernova";
...
function updateLocation() {
  ...
  var dechours =
    ut.getUTCHours() + ut.getUTCMinutes() / 60.0 + ut.getUTCSeconds() / 3600.0;
  ...
  moonAzi =
    moonpos.azimuth < Math.PI
      ? Math.PI + moonpos.azimuth
      : moonpos.azimuth - Math.PI;
  sunAlt = sunpos.altitude;
  sunAzi =
    sunpos.azimuth < Math.PI
      ? Math.PI + sunpos.azimuth
      : sunpos.azimuth - Math.PI;
  var timesofday = [
  ...
  sunmoontxt.innerHTML =
    sunriseStr + ", " + moonPhaseIcon + " " + moonPhaseDesc;
}
...
function geoFindMe() {
  ...
  message.innerHTML =
    "<img style='vertical-align:-26%; padding-right:3px; width:16px' src='" +
    urlbase +
    "wp-content/plugins/transient-table/loading.gif'>Finding your location...";
...
function nameLinked(row, type, full, meta, namefield, aliasfield) {
  ...
  if (row[aliasfield].length > 1) {
    ...
    return (
      ...
      urlstem +
      nameToFilename(row[namefield]) +
      "/' target='_blank'>" +
      noBreak(row[namefield]) +
      "</a><span class='tooltiptext'> " +
      aliases.map(noBreak).join(", ") +
      "</span></div>"
    );
  } else {
    return (
...
function nameSwitcher(data, type, row, meta, namefield, aliasfield) {
  ...
  if (type === "display" || type === "sort") {
    if (row[aliasfield] !== undefined && row[aliasfield].length > 1) {
      ...
      var filterTxt = searchFields["search"]
        .val()
        .toUpperCase()
        .replace(/"/g, "");
      var idObjTxt =
        idObj === null ? "" : idObj.value.toUpperCase().replace(/"/g, "");
...
function hostLinked(row, type, full, meta) {
  var host =
    "<a class='" +
    ("kind" in row.host[0] && row.host[0]["kind"] == "cluster"
      ? "hci"
      : "hhi") +
    "' href='" +
    urlstem +
    ...
    "/' target='_blank'></a>&nbsp;";
  var mainHost =
    "<a href='http://simbad.u-strasbg.fr/simbad/sim-basic?Ident=" +
    encodeURIComponent(row.host[0]["value"]) +
    "&submit=SIMBAD+search' target='_blank'>" +
    someBreak(row.host[0]["value"]) +
    "</a>";
  var hostImg =
    row.ra && row.dec
...
function hostSwitcher(data, type, row, meta) {
  ...
  if (type === "display" || type === "sort") {
    if (hlen > 1) {
      var mainHost =
        "<a href='http://simbad.u-strasbg.fr/simbad/sim-basic?Ident=%s&submit=SIMBAD+search' target='_blank'>%s</a>";
      var hostImg =
        row.ra && row.dec
          ? "<div class='tooltipimg' " +
            "style=background-image:url(" +
      ...
      for (var t = 0; t < tlen; t++) {
        ...
        if (txt !== "") {
          ...
          return (
            "<div class='tooltip'>" +
            host +
            mainHost.replace(/%s/g, primaryname) +
            "<span class='tooltiptext'> " +
            hostImg +
            "AKA: " +
            otheraliases.join(", ") +
...
function typeLinked(row, type, full, meta) {
  ...
  if (clen > 1) {
    ...
    return (
      "<div class='tooltip'>" +
      noBreak(row.claimedtype[0]["value"]) +
      "</a><span class='tooltiptext'> " +
      altTypes +
      "</span></div>"
    );
...
jQuery.extend(jQuery.fn.dataTableExt.oSort, {
  "non-empty-string-asc": function (str1, str2) {
    if (isNaN(str1) && isNaN(str2)) return 0;
    ...
    for (var i = 0; i < sslen; i++) {
      if (splitString[i].indexOf("-") !== -1) {
        ...
        if (minStr !== "") {
          if (
            !(
              (minStr !== "" && compDates(cData, minStr, true)) ||
              (maxStr !== "" && compDates(maxStr, cData, true)) ||
              cData === ""
            )
          )
            return !isNot;
...
function advancedRaDecFilter(data, id, pmid) {
  ...
  for (var d = 0; d < sdlen; d++) {
    ...
    var sslen = splitString.length;
    for (var i = 0; i < sslen; i++) {
      var cleanString = splitString[i]
        .trim()
        .replace(/\s+/g, ":")
        .replace(/[hm]/g, ":");
      if (!ra)
        cleanString = cleanString.replace(/[dm]/g, ":").replace(/s$/, "");
      ...
      if (cleanString.indexOf("-") !== -1) {
        ...
        if (minStr !== "") {
          if (
            !(
              (minStr !== "" && compRaDecs(cData, minStr, true)) ||
              (maxStr !== "" && compRaDecs(maxStr, cData, true)) ||
              cData === ""
            )
          )
            return !isNot;
        ...
        if (ra && minCoord < 0.0) {
          if (
            (cCoord >= minCoord && cCoord <= maxCoord) ||
            cCoord >= 360.0 + minCoord
          ) {
            return !isNot;
          } else {
            return isNot;
...
function needAdvanced(str) {
  ...
  return advancedStrs.some(function (v) {
    return str === v;
  });
}
/*]]>*/
